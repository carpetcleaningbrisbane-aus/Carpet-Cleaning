...
export default function ContactPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F2F8FC]">
      ...
      <section className="px-5 md:px-16 max-w-[1280px] mx-auto pb-24">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">

          <div className="lg:col-span-2 flex flex-col gap-6 order-2 lg:order-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
              {contactItems.map((item) => (
                <div key={item.label} className="bg-white rounded-2xl border border-[#1261A0]/20 p-5 flex items-start gap-4 ambient-shadow hover:border-[#00B8D9]/40 transition-all">
                  <div className="w-11 h-11 rounded-xl bg-[#00B8D9]/10 flex items-center justify-center text-[#00B8D9] shrink-0">
                    {item.icon}
                  </div>
                  <div>
            ...
            <div className="rounded-2xl bg-[#082B59] p-7 text-white relative overflow-hidden">
              ...
              <div className="px-8 md:px-10 py-8">
                {status === 'success' ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <button type="submit" disabled={status === 'submitting'} className="w-full py-4 bg-[#082B59] text-white font-bold rounded-xl text-sm hover:bg-[#00B8D9] hover:text-[#082B59] transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                      {status === 'submitting' ? (
