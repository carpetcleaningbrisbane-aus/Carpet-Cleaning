...
export default function Navbar() {
  ...
  return (
    <>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`font-semibold text-sm tracking-wider uppercase transition-colors duration-300 ${isActive
      ...
      <div
        className={`fixed inset-y-0 left-0 z-[60] flex flex-col p-6 h-full w-80 bg-[#f4faff] shadow-2xl transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                onClick={() => setIsOpen(false)}
                className={`flex items-center gap-4 p-3 rounded-lg font-semibold text-sm transition-all duration-200 ${isActive
                    ? 'bg-[#b1e8fa] text-[#316a79]'
                    : 'text-[#43474d] hover:bg-[#e9f6fd]'
                  }`}
              >
                {link.name === 'Home' && <Home className="w-5 h-5" />}
                {link.name === 'Services' && <Briefcase className="w-5 h-5" />}
